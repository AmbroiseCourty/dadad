...
class Application(tk.Tk):
    ...
    def show_login_page(self):
        ...
        label_welcome = tk.Label(self, text="Bienvenue dans l'interface de gestion de l'inventaire")
        label_welcome.pack(pady=10)

        label_username = tk.Label(self, text="Nom d'utilisateur")
        ...
        button_login = tk.Button(self, text="Connexion", command=self.login)
        button_login.pack(pady=5)

        button_register = tk.Button(self, text="Inscription", command=self.register)
    ...
    def show_inventory_page(self):
        ...
        if self.current_user == "admin":
            ...
            # admin_menu.add_command(label="Ajouter un utilisateur", command=self.add_users) # MARCHE PAS A CHANGER 
            admin_menu.add_command(label="Supprimer un utilisateur", command=self.delete_users)

        label_inventory = tk.Label(self, text="Inventaire")
    ...
    def afficher_users(self):
        try:
            ...
            users_only= users[['username']]
            new_window = tk.Toplevel(self)
            new_window.title("Liste des utilisateurs")

            tree = ttk.Treeview(new_window, columns= ("Nom d'utilisateur") , show = "headings")
            tree.heading("Nom d'utilateur", text="Nom d'utilisateur")
